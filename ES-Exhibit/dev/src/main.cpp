...
void drawHome() {
  ...
  display.println("4: Custom message");   // === COMPOSE: added
...
namespace Compose {
  ...
  // Classic phone mapping (multi-tap sequences)
  ...
  void backspace() {
    if (activeKey != 0) { // cancel composing char first
      activeKey = 0;
      activeIdx = 0;
      dirty = true;
      return;
  ...
  void drawWrapped(const String& s, int x, int y, uint8_t charsPerLine, uint8_t maxLines) {
    ...
    while (start < s.length() && line < maxLines) {
      ...
      if (nl >= 0 && (uint16_t)(nl - (start - len)) == 0) start++; // skip newline
      line++;
    }
  }
  ...
  void render() {
    ...
    if (activeKey != 0 && preview.length() < MAX_MSG_LEN) preview += '_'; // caret
  ...
  void handleMultiTap(char k) {
    ...
    if (activeKey == k && (now - lastTap) <= TAP_TIMEOUT) {
      ...
      commitActiveIfAny();                   // commit previous
      activeKey = k;                         // start new
      activeIdx = 0;
      dirty = true;
    }
    lastTap = now;
