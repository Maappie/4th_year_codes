...
bool initLoRaRadio() {
    // Initialize SPI and LoRa radio
    SPI.begin(18, 19, 23);  // SCK, MISO, MOSI
    LoRa.setPins(PIN_LORA_SS, PIN_LORA_RST, PIN_LORA_DIO0);
    bool success = LoRa.begin(LORA_FREQ);
    if (!success) {
        // Display error on OLED if LoRa init fails
...
bool loraSendEncrypted(const String& plaintext) {
    ...
    bool ok = aesGcmEncrypt(txKey.key, pbuf.get(), plen, nonce, NONCE_LEN,
                             aad, aad_len, cbuf.get(), tag, TAG_LEN);
    if (!ok) {
        Serial.println("[ENC] AES-GCM encrypt failed");
        return false;
    }
    String frame = "ENC|tag=";
    frame += TX_SENDER_TAG;
    frame += "|n="; frame += nHex;
...
void handleLoRaRx() {
    ...
    if (packetSize) {
        ...
        String shown = incoming;  // default to incoming text (plaintext)
